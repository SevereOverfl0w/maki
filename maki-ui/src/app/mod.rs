...
impl App {
    ...
    fn dispatch_overlay(&mut self, key: KeyEvent) -> Option<Vec<Action>> {
        ...
        if Key::from_event(key).is_some_and(|k| self.float_mgr.handle_focused_key(k)) {
            return Some(vec![]);
        }
    ...
    fn handle_key(&mut self, key: KeyEvent) -> Vec<Action> {
        ...
        if Key::from_event(key).is_some_and(|k| self.float_mgr.handle_claimed_key(k)) {
            return vec![];
        }
    ...
    /// A key no notation names is nothing a plugin could have bound, so it is
    /// untaken here and falls through to the host.

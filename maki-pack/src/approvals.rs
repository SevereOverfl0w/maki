...
pub struct Entry {
    ...
    /// user approved it. An entry written before egress was recorded has none,
    /// so a package that now declares any has widened its reach and is asked
    /// about again rather than inheriting an answer nobody was given.
    #[serde(default)]
...
impl Approvals {
    ...
    pub fn approve(&mut self, key: &ApprovalKey, permissions: Vec<String>, net_hosts: Vec<String>) {
        self.entries.insert(
            key.name.clone(),
            Entry {
                src: key.src.clone(),
...
mod tests {
    ...
    fn round_trips_through_json() {
        ...
        assert_eq!(entry.net_hosts(), [HOST.to_owned()]);
    }

    /// Files written before hosts were recorded still load, and grant no host,
    /// so a package that declares any is asked about again.
    #[test]
    fn an_entry_saved_before_hosts_existed_still_loads() {
        ...
        assert_eq!(entry.permissions(), ["net".to_owned()]);
        assert!(entry.net_hosts().is_empty());
    }
}

...
impl Requested {
    ...
    /// The hosts the manifest asks to reach. A request, like every other name
    /// in here, so the approval store is what decides whether they are granted.
    pub fn net_hosts(&self) -> &[String] {
        self.0.net_hosts.as_deref().unwrap_or_default()
    }

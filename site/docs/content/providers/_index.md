...
The value is a free-form JSON object. Maki decides where it lives and who can read it, the plugin decides what is in it. Each slug is one file at `~/.local/state/maki/auth/<slug>.json`, created with mode 0600, replaced in a single atomic step, and locked against other Maki processes touching the same provider. A plugin can only reach slugs it registered itself.
